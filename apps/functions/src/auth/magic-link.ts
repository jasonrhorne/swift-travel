...
async function checkRateLimit(
  ...
  await redis.set(key, newCount.toString(), {
    ex: authConfig.rateLimitWindowMinutes * 60,
  });
...
export const handler: Handler = async event => {
  ...
  try {
    ...
    if (!rateLimit.allowed) {
      ...
      return createAuthResponse(
        ...
        },
        {
          ...
          'X-RateLimit-Reset': (
            Date.now() +
            authConfig.rateLimitWindowMinutes * 60 * 1000
          ).toString(),
        }
      );
    }
    ...
    return createAuthSuccessResponse(response, 200, {
