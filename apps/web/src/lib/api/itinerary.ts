...
class ItineraryAPI {
  ...
  ): Promise<ItinerarySubmissionResponse> {
    try {
      ...
      return {
        ...
        error: {
          code: 'UNKNOWN_ERROR',
          message:
            'An unexpected error occurred while submitting your requirements.',
        },
      };
    }
  }
  ...
  async getRequestStatus(requestId: string): Promise<ItineraryStatusResponse> {
    try {
      const response = await this.request<ItineraryStatusResponse>(
        `/itineraries-status?requestId=${encodeURIComponent(requestId)}`
      );
      ...
      return {
        ...
        error:
          error instanceof ItineraryAPIError
            ? error
            : new Error('Failed to cancel request'),
      };
    }
  }
